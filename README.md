...
`meta.title` 默认原样显示：`{ title: 'abc' }` 显示 `abc`，即使存在同名翻译键也不会自动翻译。需要国际化时，从 `@/locales` 导入 `$t` 并显式写为 `{ title: $t('menu.dashboard') }`，该写法在配置求值时确定文案；若需随语言切换更新，使用 `{ get title() { return $t('menu.dashboard'); } }`（框架内置路由采用此写法）。菜单、面包屑、标签页和浏览器标题直接使用求值后的文案，后端下发的标题也按原文显示。
